...
//executes in THE thread it's gonna stoppublic class ServerThreadTest
...
public class ServerThreadTest
{
    public ServerThreadTest() {
        ...
        IoC.Resolve<Lib.ICommand>("Scopes.Current.Set",
            IoC.Resolve<object>("Scopes.New", 
                IoC.Resolve<object>("Scopes.Root")            
            )
        ).Execute();

        IoC.Resolve<Lib.ICommand>("IoC.Register", 
            "Hard Stop The Thread", 
            (object[] args) => {
                return new UActionCommand( () => 
                {
                    new HardStopCommand((ServerThread) args[1]).Execute();
    ...
    public void HardStopCommandShouldStopServerThread() {
        ...
        // Arrange
        // new Queue
        var q = new BlockingCollection<Lib.ICommand>(10);
        // new St
